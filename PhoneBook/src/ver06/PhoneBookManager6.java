...
public class PhoneBookManager6 {
	...
	// 20.10.23 ArrayList로 수정
	// 정보 저장을 위한 배열선언: 상속 관계이므로 PhoneInfor타입으로 선언
	ArrayList<String> phoneBook = new ArrayList<String>(); // 전화번호 정보를 저장할 배열
	private int cnt = 0; // 배열에 저장된 정보(요소)의 개수, 배열의 인덱스

	String pNum;
	phoneBook.add(new String(pNum));
	...
	private static PhoneBookManager6 manager =
	
	//20.10.21 추가
	// 외부에서 참조변수를 받을 수 있는 메소드
	// 내부에서만 처리하려면 public 처리 안해도 됨. 하지만 보통 싱글톤 처리를 하면 public으로 처리 
	public static PhoneBookManager6 getInstance() {
		return manager;
	}
	...
	private void addInfor(PhoneInfor6 info) { // 반환값 없음.
...
//			break;
		case Menu.UNIV:
			...
			addInfor(new PhoneUnivInfor6(name, phoneNumber, address, email, major, grade));
			break;
		case Menu.COM:
			// 추가 정보를 받고 -> 인스턴스 생성 -> 배열에 저장
			...
			addInfor(new PhoneCompanyInfor6(name, phoneNumber, address, email, company));
			break;
		case Menu.CAFE:
			// 추가 정보를 받고 -> 인스턴스 생성 -> 배열에 저장
			...
			addInfor(new PhoneCafeInfor6(name, phoneNumber, address, email, cafeName, nickName));
