...
//	new PhoneBookManager3;
	...
	// 정보 저장을 위한 배열선언
	final PhoneInfor3[] phoneBook; // 인스턴스 변수로 저장 -> 메소드들이 PHoneInfor[] phonebook을 대상으로 처리가 됨
	...
	// 초기화(생성자 이용)
	public PhoneBookManager3(){
		//phonebook = new PhoneInfor[100];	// PhoneInfor 배열의 저장공간을 100개 생성 // 2
		this(100); 		// size에 100이 들어감.
	}
	
	// 생성자의 오버로딩 이용
	PhoneBookManager3(int size) {
		phoneBook = new PhoneInfor3[size]; //3
...
//	 저장 기능 //4
	...
	void insertMember() { // 사용자로부터 데이터를 받기 때문에 매개변수는 필요 x 
		...
		String phoneNum =Util3.sc.nextLine();
		
		// 생일 받기
		System.out.println("생일 >> ");
		String birthday = Util3.sc.nextLine().trim(); // trim -> 앞뒤 공백을 지운 후 값을 저장
		
		
//		인스턴스 생성 // 6
		// 배열에 저장하기 위한 인스턴스의 참조 변수 선언
		PhoneInfor3 infor = null;
		
		// 생일이 입력되었는지 여부에 따라 인스턴스 생성을 다르게 처리
		if(birthday != null && birthday.length()>0) { // null 값이 아니고 length가 0보다 크다 	
			infor = new PhoneInfor3(name, phoneNum, birthday);
		} else {
			infor = new PhoneInfor3(name, phoneNum);			
		}
