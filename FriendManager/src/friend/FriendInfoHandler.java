...
public class FriendInfoHandler {
		...
		// 친구정보를 저장하는 배열
		private Friend[] friends; // 외부에서 접근 차단 => 친구의 정보를 저장하는 배열
		private int numOfFriends = 0; // 저장된 친구의 정보 개수, 배열의 요소 추가시 인덱스로 사용
...
//		}
		...
		// choice -> 고교 친구: 1 / 대학친구: 2
		void addFriend(int choice) {		
			
			// 사용자로 부터기본 정보를 입력받자: 이름, 전화번호, 주소
			...
			if(choice==1) {
				...
				// 대학친구의 인스턴스를 배열에 저장
				addFriendInfo(new UniverFriend(name, pNum, addr, major));
			}
		...
		public void showAllData() {
			...
			for(int i=0; i<numOfFriends; i++) {
				friends[i].showData(); // friends[0] -> Friend 타입의 참조변수 = 하위 클래스의 인스턴스들을 참조
				System.out.println("----------");
			}
		}
