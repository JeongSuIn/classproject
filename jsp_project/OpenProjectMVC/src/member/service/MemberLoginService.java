...
public class MemberLoginService {
	...
	public void memberLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		// id, pw를 조건으로 Member 테이블에서 select한 결과가 있으면 인증에 성공, 없으면 인증 실패
		// 로그인 여부 체크
		...
		try {
			...
			if (member != null) {
				...
				// session이 있으면 session을 반환, 없으면 새로운 session객체를 생성해서 반환
				// getSession() -> false가 들어가는 경우 -> session이 있으면 session을 반환, 없으면 null을 반환
				// getSession() -> true가 들어가는 경우 -> 무조건 새로운 세션을 생성해서 반환
