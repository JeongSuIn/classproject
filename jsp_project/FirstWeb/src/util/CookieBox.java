...
// 쿠키 객체를 생성하고 저장하고, 저장된 쿠키를 꺼내 사용하는 기능

public class CookieBox {
	//쿠키 객체를 저장하는 Map 객체를 생성
	...
	public String getValue(String name) throws IOException {
		Cookie cookie = cookieMap.get(name);	// Map에 name의 키가 없는 경우 null 반환도 가능
		if(cookie==null) {
			return null;
		}
	...
	// 이름, 값, 경로, 유지 시간, 도메인을 가지고 Cookie객체를 생성
		public static Cookie createCookie(
			...
			Cookie cookie = new Cookie(name, value);
			cookie.setPath(path); // 경로 설정
			cookie.setMaxAge(maxAge); // 유지 시간 설정
